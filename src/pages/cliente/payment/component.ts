...
export class ListComponent implements OnInit {
    ...
    public montoACobrar:number = 0;
    public paymentMovement:string = null;
    public sala:string = null;
    public tokenMovement:string = null;
    public isMobile:boolean = false;
    // Socket connection 
    private socketSubscription: Subscription;
    public onNewMessage(): Observable<any> {
        return new Observable<any>(observer => {
            this.Socket.on('requirePayment', (data: any) => {
                if(!this.paymentMovement){
                    this.montoACobrar = data.total_amount;
                    this.paymentMovement = data.uuid;
                    this.tokenMovement = data.token;
                }else{
                    //generar cancelación de nuevo cobro
                    Swal.fire({
                        title: "Error",
                        text: "Ya tienes una operación de cobro en proceso, por favor concluye la operación para poder realizar otra",
                        icon: "error",
            ...
            this.Socket.on('paymentComplete', (data: any) => {
                console.log(data + " " + this.paymentMovement);
                if(this.paymentMovement == data){
                    this.getWallet();
                    this.montoACobrar = 0;
                    this.paymentMovement = null;
                }
            })
            this.Socket.on('cancelPayment', (data: any) => {
                if(this.paymentMovement == data){
                    ...
                    Swal.fire({
                        title: "Error",
                        text: "Orden de pago cancelada por el emisor",
                        icon: "warning",
                        confirmButtonText: "Entendido",
                    });
                }
    ...
    public SocketConnectionId:string = null;
    ...
    async getProfile(){
        this.loading = true;
        this.ServiceUser.getProfile({}).subscribe((response: any) => {
            this.loading = false;
            console.log(response);
            if(response.ok && response.body.success == 1){
                let data = response.body.data;
                this.sala = data.uuid;
                this.conexionSocket(this.sala);
            }else{
                Swal.fire({
                    title: "Error",
    ...
    cancelarOperacion(){
        if(this.montoACobrar > 0 && this.paymentMovement != null){
            Swal.fire({
                icon: 'warning',
                title: '¿Estás seguro?',
                ...
                if (result.isConfirmed) {
                    this.loading = true;
                    this.ServiceUser.cancelQrCharge({chanel:this.tokenMovement}).subscribe((response: any) => {
                        console.log(response);
                        this.loading = false;
                        if(response.ok && response.body.success == 1){
                            this.Socket.emit('cancelPayment', {sala:this.sala,uuid:this.paymentMovement});
                            Swal.fire({
                                title: "Exito",
                                text: 'Tu operación se ha cancelado correctamente',
                                icon: "success",
                                confirmButtonText: "Entendido",
                            });
                            this.getWallet();
            ...
            Swal.fire({
                title: "Error",
                text: "No existe una operación de cobro para cancelar",
                icon: "error",
                confirmButtonText: "Entendido",
            });
        }
    ...
    generarPago(){
        if(!this.montoACobrar || this.montoACobrar == 0){
            Swal.fire({
                title: "Error",
        ...
        let code = values.join('');
        this.loading = true;
        this.ServiceUser.pagarQrCharge({uuid:this.paymentMovement,code:code}).subscribe((response: any) => {
            console.log(response);
            this.loading = false;
            if(response.ok && response.body.success == 1){
                this.Socket.emit('paymentComplete',  {sala:this.sala,uuid:this.paymentMovement});
                this.Socket.emit('leavePayment', this.sala);
                Swal.fire({
                    title: "Exito",
